...
export async function DiscordRequest(endpoint: string, options: DiscordRequestOptions): Promise<Response> {
  ...
  if (!DISCORD_API_URL || !DISCORD_TOKEN) {
    throw new Error("Missing DISCORD_API_URL or DISCORD_TOKEN environment variable");
  }
...
export async function InstallGlobalCommands(appId: string, commands: APIApplicationCommand[]): Promise<void> {
